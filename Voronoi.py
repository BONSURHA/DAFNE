...
# Funzione per unire casualmente i tasselli adiacenti
def merge_adjacent_cells(voronoi_cells, num_merges):
    for _ in range(num_merges):
        site1, site2 = random.sample(list(voronoi_cells.keys()), 2)

        # Unisci le celle dei siti site1 e site2
        voronoi_cells[site1] += voronoi_cells[site2]
        del voronoi_cells[site2]
...
def split_and_save_fragments(url, output_directory, num_merges):
    ...
    # Unisci casualmente i tasselli adiacenti prima di creare i frammenti
    merge_adjacent_cells(voronoi_cells, num_merges)
...
if __name__ == "__main__":
    ...
    num_merges = 10  # Numero di unione casuale dei tasselli adiacenti

    split_and_save_fragments(url, output_directory, num_merges)
