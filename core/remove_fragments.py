...
def add_spurious_fragments(path, spurious_path, num_spurious_fragments, num_fragments):

    this_num_spurious = random.randint(1, num_spurious_fragments)
    sys.stderr.write(f'\r\nadd {this_num_spurious} spurious fragments\n')
    fragments = []
    name_used = []
    fragments_path = os.path.join(path, "fragments")
    resources_path = os.path.join(path, "resources")

    spurious_path_fragments = os.path.join(spurious_path, "fragments")

    img_extension = ['.jpg', '.jpeg', '.png']

    for filename in os.listdir(spurious_path_fragments):
        if filename.endswith(tuple(img_extension)):
            fragments.append(os.path.join(spurious_path_fragments, filename))


    spurious_fragments = random.sample(fragments, this_num_spurious)
    for i, spurious_fragments in enumerate(spurious_fragments):
        name = f"fragment_{num_fragments+i+1}"
        name_used.append(name)
        image = Image.open(spurious_fragments)
        file_path = os.path.join(fragments_path, f"{name}.png")
        image.save(file_path, 'PNG')


    
    with open(f"{resources_path}/spurious_info.txt", "a") as info_file:
        info_file.write(f"num_spurious: {this_num_spurious}\n")
        for name in name_used:
            info_file.write(f"name_spurious: {name}\n")
    sys.stderr.write(f'\rdone\n')


def generate_info(resources_path, seed, removal_percentage, num_spurious):
    with open(f"{resources_path}/removal_info.txt", "a") as info_file:
        ...
        info_file.write(f"num_spurious: {num_spurious}\n")
...
def main():
    ...
    parser.add_argument('--spurious_directory', type=str, help='Path to the folder generated by DAFNE, the outermost one, in order to derive the spurious fragments', required= False)
    ...
    if args.file_path is not None:
        ...
        if "num_spurious" in input_data:
            num_spurious = int(input_data.get("num_spurious"))


    path, num_fragments = random_fragments_removal(seed, input_directory, output_directory, removal_percentage, original_image)

    if num_spurious !=0 and args.spurious_directory is not None:
        spurious_directory = args.spurious_directory
        add_spurious_fragments(path, spurious_directory, num_spurious, num_fragments)
    else:
        sys.stderr.write('\r\nskip spurious operation\n')
        sys.stderr.write('\rparameter missing: need "num_spurious", int > 0, in the text file and folder to select the spurious\n')
